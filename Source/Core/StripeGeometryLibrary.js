// Modified from cesium CorridorGeometryLibrary.js
...
class StripeGeometryLibrary {
  static computeRoundCorner(
    cornerPoint,
    startPoint,
    endPoint,
    cornerType,
  ...
  static addEndCaps(calculatedPositions) {
    ...
    const firstEndCap = StripeGeometryLibrary.computeRoundCorner(
      cornerPoint,
      startPoint,
      endPoint,
      CornerType.ROUNDED,
    ...
    cornerPoint = Cartesian3.midpoint(startPoint, endPoint, cornerPoint);
    const lastEndCap = StripeGeometryLibrary.computeRoundCorner(
      cornerPoint,
      startPoint,
      endPoint,
      CornerType.ROUNDED,
  ...
  static addShiftedPositions(positions, left, scalar, calculatedPositions) {
  ...
  static addAttribute(attribute, value, front, back) {
    const x = value.x;
    const y = value.y;
    const z = value.z;
    if (defined(front)) {
  ...
  static computePositions(params) {
    ...
    let f = new Cartesian3();
    let l = new Cartesian3();
    let anotherComputedPositions = [];
    for (let i = 1; i < positions.length; i++) {
      const prePosition = positions[i - 1];
      const curPosition = positions[i];
      n = ellipsoid.geodeticSurfaceNormal(prePosition, n);
      f = Cartesian3.normalize(
        Cartesian3.subtract(curPosition, prePosition, f),
        f
      );
      l = Cartesian3.normalize(Cartesian3.cross(n, f, l), l);
      anotherComputedPositions = StripeGeometryLibrary.addShiftedPositions(
        [
          prePosition.x,
          prePosition.y,
          prePosition.z,
    ...
    for (i = 1; i < length - 1; i++) {
      ...
      if (doCorner) {
        ...
        if (leftIsOutside) {
          ...
          calculatedPositions = StripeGeometryLibrary.addShiftedPositions(
            [
              previousPos.x,
              previousPos.y,
              previousPos.z,
          ...
          if (
            ...
            let lPos = new Cartesian3();
            lPos = Cartesian3.add(
              position,
              Cartesian3.multiplyByScalar(left, width, lPos),
              lPos
            );
            corners.push({
              leftPositions: [lPos.x, lPos.y, lPos.z],
            });
          } else {
            corners.push({
              leftPositions: StripeGeometryLibrary.computeMiteredCorner(
                position,
          ...
          if (
            ...
            let rPos = new Cartesian3();
            rPos = Cartesian3.add(
              position,
              Cartesian3.negate(
                Cartesian3.multiplyByScalar(left, width, rPos),
                rPos
              ),
              rPos
            );
            corners.push({
              rightPositions: [rPos.x, rPos.y, rPos.z],
            });
          } else {
            corners.push({
              rightPositions: StripeGeometryLibrary.computeMiteredCorner(
                position,
                cornerDirection,
                rightPos,
                leftIsOutside
    ...
    if (cornerType === CornerType.ROUNDED) {
      endPositions = StripeGeometryLibrary.addEndCaps(calculatedPositions);
    }
    return {
...
export default StripeGeometryLibrary;
