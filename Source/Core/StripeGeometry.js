// Modified from cesium CorridorGeometry.js
import StripeGeometryLibrary from "./StripeGeometryLibrary";
...
function addNormals(attr, normal, left, front, back, vertexFormat) {
  ...
  if (vertexFormat.normal) {
    StripeGeometryLibrary.addAttribute(normals, normal, front, back);
  }
  if (vertexFormat.tangent) {
    StripeGeometryLibrary.addAttribute(tangents, forward, front, back);
  }
  if (vertexFormat.bitangent) {
    StripeGeometryLibrary.addAttribute(bitangents, left, front, back);
  }
}
function combine(computedPositions, vertexFormat, ellipsoid) {
  ...
  if (addEndPositions) {
    ...
    for (i = 0; i < halfLength; i++) {
      ...
      StripeGeometryLibrary.addAttribute(finalPositions, rightPos, front);
      StripeGeometryLibrary.addAttribute(
        finalPositions,
        leftPos,
        undefined,
        back
  ...
  for (i = 0; i < corners.length; i++) {
    ...
    if (defined(l)) {
      ...
      for (j = 0; j < r.length / 3; j++) {
        ...
        indices[index++] = start + j;
        indices[index++] = start + j + 1;
        StripeGeometryLibrary.addAttribute(finalPositions, outsidePoint, front);
        previousPoint = Cartesian3.fromArray(
          finalPositions,
          pivot * 3,
          previousPoint
...
function extrudedAttributes(attributes, vertexFormat) {
  ...
  if (vertexFormat.normal || vertexFormat.bitangent || vertexFormat.tangent) {
    ...
    for (i = 0; i < threeSize; i += 3) {
      ...
      if (vertexFormat.normal) {
        StripeGeometryLibrary.addAttribute(normals, normal, attrIndexOffset);
        StripeGeometryLibrary.addAttribute(
          normals,
          normal,
          attrIndexOffset + 3
        );
        StripeGeometryLibrary.addAttribute(normals, normal, attrIndex);
        StripeGeometryLibrary.addAttribute(normals, normal, attrIndex + 3);
      }
      if (vertexFormat.tangent || vertexFormat.bitangent) {
        ...
        if (vertexFormat.bitangent) {
          ...
          StripeGeometryLibrary.addAttribute(bitangents, bitangent, attrIndex);
          StripeGeometryLibrary.addAttribute(
            bitangents,
            bitangent,
            attrIndex + 3
        ...
        if (vertexFormat.tangent) {
          ...
          StripeGeometryLibrary.addAttribute(tangents, tangent, attrIndex);
          StripeGeometryLibrary.addAttribute(tangents, tangent, attrIndex + 3);
        }
      }
      attrIndex += 6;
    }
...
function computePositionsExtruded(params, vertexFormat) {
  ...
  const computedPositions = StripeGeometryLibrary.computePositions(params);
...
class StripeGeometry {
  /**
   * A description of a stripe. Stripe geometry can be rendered with both {@link Primitive} and {@link GroundPrimitive}.
   *
   * @alias StripeGeometry
   * @constructor
   *
   * @param {Object} options Object with the following properties:
   * @param {Cartesian3[]} options.positions An array of positions that define the center of the stripe.
   * @param {Number} options.width The distance between the edges of the stripe in meters.
  ...
  constructor(options) {
    ...
    this._workerName = "createStripeGeometry";
   ...
   *
   * @param {StripeGeometry} value The value to pack.
   ...
   * @param {StripeGeometry} [result] The object into which to store the result.
   * @returns {StripeGeometry} The modified result parameter or a new StripeGeometry instance if one was not provided.
   */
  static unpack(array, startingIndex = 0, result) {
    ...
    if (!result) {
      return new StripeGeometry({
        ...
        offsetAttribute: offsetAttribute === -1 ? undefined : offsetAttribute,
      });
    }
  ...
  /**
   * Computes the geometric representation of a stripe, including its vertices, indices, and a bounding sphere.
   *
   * @param {StripeGeometry} stripeGeometry A description of the stripe.
   * @returns {Geometry|undefined} The computed vertices and indices.
   */
  static createGeometry(stripeGeometry) {
    let positions = stripeGeometry._positions;
    const width = stripeGeometry._width;
    const ellipsoid = stripeGeometry._ellipsoid;
    positions = scaleToSurface(positions, ellipsoid);
    const cleanPositions = arrayRemoveDuplicates(
      positions,
      Cartesian3.equalsEpsilon
    ...
    const height = stripeGeometry._height;
    const extrudedHeight = stripeGeometry._extrudedHeight;
    ...
    const vertexFormat = stripeGeometry._vertexFormat;
    const params = {
      ellipsoid: ellipsoid,
      positions: cleanPositions,
      width: width,
      cornerType: stripeGeometry._cornerType,
      granularity: stripeGeometry._granularity,
      saveAttributes: true,
    };
    let attr;
    if (extrude) {
      ...
      params.shadowVolume = stripeGeometry._shadowVolume;
      params.offsetAttribute = stripeGeometry._offsetAttribute;
      ...
      if (defined(stripeGeometry._offsetAttribute)) {
        const applyOffsetValue =
          stripeGeometry._offsetAttribute === GeometryOffsetAttribute.NONE
            ? 0
            : 1;
    ...
    const options = {
      ...
      offsetAttribute: stripeGeometry._offsetAttribute,
    };
    return new Geometry(options);
  }

  static createShadowVolume(stripeGeometry, minHeightFunc, maxHeightFunc) {
    const granularity = stripeGeometry._granularity;
    const ellipsoid = stripeGeometry._ellipsoid;
    const minHeight = minHeightFunc(granularity, ellipsoid);
    const maxHeight = maxHeightFunc(granularity, ellipsoid);
    return new StripeGeometry({
      positions: stripeGeometry._positions,
      width: stripeGeometry._width,
      cornerType: stripeGeometry._cornerType,
      ellipsoid: ellipsoid,
      granularity: granularity,
